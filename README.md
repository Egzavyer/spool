...
When the server is run, it starts by applying all database migrations. Then, it creates the `ConnectRPC Service Handlers` generated by `Buf` and serves the RPCs on a `Go HTTP Server`. The procedures then interact with the database using the [jackc/pgx](https://github.com/jackc/pgx) library and return the response to the client.
