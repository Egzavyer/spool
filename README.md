...
When the server is run, it starts by applying all database migrations. Then, it creates the `ConnectRPC Service Handlers` generated by `Buf` and serves the RPCs on a `Go HTTP Server`. None of the procedures do anything for now.
